...
import com.group.gateway.common.dto.ClientInfo;
import com.group.gateway.common.dto.RateLimitResult;
import com.group.gateway.common.dto.RateLimitConfig;
import com.group.gateway.common.service.RateLimitService;
import com.group.gateway.common.service.RequestContextService;
import com.group.gateway.common.model.RequestContext;
...
public class RateLimitFilter implements GlobalFilter, Ordered {
    ...
    // 默认限流配置
    private static final long DEFAULT_IP_LIMIT = 100;        // IP每分钟限制
    private static final long DEFAULT_USER_LIMIT = 200;      // 用户每分钟限制
    private static final long DEFAULT_API_LIMIT = 500;       // API每分钟限制
    private static final long DEFAULT_TENANT_LIMIT = 1000;   // 租户每分钟限制
    private static final long DEFAULT_GLOBAL_LIMIT = 10000;  // 系统每分钟限制
    private static final long DEFAULT_LIMIT = 50;            // 默认限制
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            // 执行多维度限流检查
            return performRateLimitChecks(exchange, path, method, clientInfo, rateLimitConfig)
                .flatMap(results -> {
                    // 检查是否有任何限流超限
                    RateLimitResult exceededResult = null;
                    for (RateLimitResult result : results.values()) {
                        if (!result.isSuccess()) {
                            exceededResult = result;
                            break;
                        }
                    }
                    
                    if (exceededResult != null) {
                        return handleRateLimitExceeded(exchange, exceededResult);
                    }
                ...
                .onErrorResume(throwable -> {
                    log.error("限流检查发生错误，允许请求通过: {}", throwable.getMessage(), throwable);
                    return chain.filter(exchange);
                });
                
        } catch (Exception e) {
            log.error("限流过滤器执行异常: {}", e.getMessage(), e);
            return chain.filter(exchange);
        }
    }
    ...
    private RateLimitConfig getRateLimitConfig(String path) {
        ...
        config.setIpLimit(DEFAULT_IP_LIMIT);
        config.setUserLimit(DEFAULT_USER_LIMIT);
        config.setApiLimit(DEFAULT_API_LIMIT);
        config.setTenantLimit(DEFAULT_TENANT_LIMIT);
        config.setGlobalLimit(DEFAULT_GLOBAL_LIMIT);
        
        // 根据路径调整配置
        if (path.startsWith("/api/v1/")) {
            // API v1 限制更严格
            config.setApiLimit(DEFAULT_API_LIMIT * 2);
        } else if (path.startsWith("/admin/")) {
            // 管理接口限制更严格
            config.setApiLimit(DEFAULT_API_LIMIT / 2);
            config.setUserLimit(DEFAULT_USER_LIMIT / 2);
        }
     ...
     * 执行多维度限流检查
     */
    private Mono<Map<String, RateLimitResult>> performRateLimitChecks(
            ServerWebExchange exchange, String path, String method, 
            ClientInfo clientInfo, RateLimitConfig rateLimitConfig) {
